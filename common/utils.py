...
from pathlib import Path
from common.data_classes import CrossValidationData,InferenceValidator

project_root = Path("/app")



def _save_local_model( model):
    path = project_root / "models" / "trained_catboost_model.pkl"
    joblib.dump(model, path)
    return str(path)

def _save_training_data_to_gcs( df) -> str:
    from datetime import datetime
    ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    uri = f"gs://pennicilin_batch_yield/training_data/version={ts}/train.parquet"
    df.to_parquet(uri, index=False)
    return uri

def time_series_split(train_fraction:int,x:pd.DataFrame,y:pd.Series) -> CrossValidationData:
    """

    Args:
        train_fraction: fraction of training data from complete X input
        x: input features
        y: target variable
    ...
    """
    train_idx = int(train_fraction * len(x))
    x_tr, x_val = x.iloc[:train_idx], x.iloc[train_idx:]
    y_tr, y_val = y.iloc[:train_idx], y.iloc[train_idx:]

    cv_data = CrossValidationData(x_tr, y_tr, x_val, y_val)

    return  cv_data

def configure_mlflow(tracking_uri_key:str,registry_uri_key:str,experiment_name:str) -> MlflowClient:
        """
        Configures mlflow settings for model training.
        Uses load_dotenv to load environment variables to set tracking and registry URIs.

        Args:
            tracking_uri_key: URI of mlflow tracking server
            registry_uri_key: URI of mlflow registry
            experiment_name: MLFlow experiment name

        Returns:
            MLflowClient with tracking and registry UIs set
...
def load_model(base_path: Path) -> Any:
    """
    ...
    cbm_path = base_path.with_suffix(".cbm")
